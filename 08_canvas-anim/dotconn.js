...
function paintSlide(timestamp) {
    ...
    console.log("drew circle radius "+radius);
      ...
      expand = true;
		...
		logo_x += logo_vx;
		logo_y += logo_vy;
		if(logo_x <= 0 || logo_x >= 600 - logo.naturalWidth*scale){
			logo_vx *= -1;
		}
	    if(logo_y <= 0 || logo_y >= 600 - logo.naturalHeight*scale){
			logo_vy *= -1;
		}
	    ctx.drawImage(logo,logo_x,logo_y,logo.naturalWidth*scale,logo.naturalHeight*scale);
